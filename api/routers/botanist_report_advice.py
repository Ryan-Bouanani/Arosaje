...
router = APIRouter(prefix="/botanist-report-advice", tags=["botanist-report-advice"])
...
async def create_botanist_advice_on_report(
    ...
    report = (
        ...
        .filter(CareReportModel.id == advice.care_report_id)
        .first()
    )
    if not report:
    ...
    return crud_advice.create_botanist_report_advice(
        db=db, advice=advice, botanist_id=current_user.id
    )


@router.get(
    "/care-report/{care_report_id}",
    response_model=List[BotanistReportAdviceWithDetails],
)
