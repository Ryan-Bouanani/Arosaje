...
def create_botanist_report_advice(
    db: Session, advice: BotanistReportAdviceCreate, botanist_id: int
...
def get_botanist_advices_by_care_report(
    db: Session, care_report_id: int
) -> List[BotanistReportAdvice]:
    """Récupérer tous les avis pour un rapport donné"""
    return (
        db.query(BotanistReportAdvice)
...
def get_botanist_advices_by_botanist(
    db: Session, botanist_id: int
) -> List[BotanistReportAdvice]:
...
def update_botanist_report_advice(
    db: Session, advice_id: int, advice_text: str, botanist_id: int
) -> BotanistReportAdvice:
    ...
    db_advice = (
        ...
        .filter(
            ...
            BotanistReportAdvice.botanist_id
            == botanist_id,  # S'assurer que c'est bien son avis
        )
        .first()
    )
    ...
    if not db_advice:
        raise ValueError(
            "Avis non trouvé ou vous n'avez pas l'autorisation de le modifier"
        )
...
def get_my_advice_for_report(
    db: Session, care_report_id: int, botanist_id: int
) -> BotanistReportAdvice:
...
def get_care_reports_with_my_advice(db: Session, botanist_id: int):
    ...
    return (
        ...
        .join(
            BotanistReportAdvice, CareReport.id == BotanistReportAdvice.care_report_id
        )
        .filter(BotanistReportAdvice.botanist_id == botanist_id)
