...
class GoogleGeocodingService:
    ...
    def __init__(self):
        ...
        self.api_key = os.getenv(
            "GOOGLE_GEOCODING_API_KEY", "AIzaSyDGGp6USx0kYrG1DtIE6g7_UKmz7p4ARcM"
        )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(self.base_url, params=params) as response:
                    ...
                    if data.get("status") != "OK":
                        logger.warning(
                            f"Géocodage échoué pour '{address}': {data.get('status')}"
                        )
                    ...
                    logger.info(
                        f"Géocodage réussi: '{formatted_address}' -> ({lat}, {lng})"
                    )
    ...
    async def _geocode_fallback(self, location: str) -> Optional[Tuple[float, float]]:
        ...
        if location_lower.startswith("75"):
            logger.info(
                "Code postal parisien détecté, utilisation des coordonnées de Paris"
            )
