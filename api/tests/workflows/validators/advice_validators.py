def validate_advice_response(response, expected_status=None):
    ...
    if expected_status:
        assert (
            data["validation_status"] == expected_status
        ), f"Le statut attendu était {expected_status}, mais a reçu {data['validation_status']}"

    # Vérification des valeurs par défaut
    assert data["version"] >= 1, "La version doit être >= 1"
    assert isinstance(
        data["is_current_version"], bool
    ), "is_current_version doit être un boolean"
    assert data["priority"] in [
        "normal",
        "urgent",
        "follow_up",
    ], "Priority doit être valide"
