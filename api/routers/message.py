...
from schemas.message import Message, MessageCreate, Conversation, ConversationCreate
...
async def get_conversation_messages(
    ...
    return message.get_conversation_messages(
        db, conversation_id=conv_id, skip=skip, limit=limit
    )
...
async def create_message(
    ...
    try:
        ...
        new_message = message.create_message(
            db=db, message=message_in, sender_id=current_user.id
        )
...
def get_unread_messages_count(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Erreur lors du comptage des messages non lus"
        )
...
async def send_message(
    ...
    if not any(p.id == current_user.id for p in participants):
        raise HTTPException(
            status_code=403, detail="Vous ne participez pas à cette conversation"
        )

    # Créer le message
    new_message = message.create_message(
        db=db, message=message_data, sender_id=current_user.id
    )
...
async def create_botanist_conversation(
    ...
    if not botanists:
        raise HTTPException(
            status_code=404, detail="Aucun botaniste disponible pour le moment"
        )
