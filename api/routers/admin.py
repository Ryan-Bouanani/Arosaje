...
async def send_validation_email(user_email: str, user_name: str):
    ...
    try:
        await email_service.send_account_validated_email(
            recipient_email=user_email, user_name=user_name
        )
...
async def reject_user(
    ...
    # Envoyer l'email en arrière-plan
    background_tasks.add_task(
        send_rejection_email, user_email=user_email, user_name=user_name
    )
...
async def get_admin_stats(
    ...
    try:
        ...
        active_cares = (
            db.query(PlantCare)
            .filter(PlantCare.status.in_(["IN_PROGRESS", "ACCEPTED"]))
            .count()
        )
...
async def change_user_role(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Rôle invalide: {new_role}"
        )
