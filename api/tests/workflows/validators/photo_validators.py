...
def verify_photo_list(response: Any, expected_count: int = None) -> None:
    ...
    if expected_count is not None:
        assert (
            len(photos) == expected_count
        ), f"Le nombre de photos attendu est {expected_count}, reçu {len(photos)}"
...
def validate_photo_response(response: Any) -> None:
    ...
    assert (
        "created_at" in json_response
    ), "La réponse doit contenir une date de création"
