    ...
    try:
        ...
        try:
            ...
            await monitoring_service.track_api_request(
                request, response, duration, user_id
            )
            ...
            if user_id:
                ...
                if len(path_parts) > 0:
                    feature = path_parts[
                        0
                    ]  # Utiliser la première partie du chemin comme nom de fonctionnalité
                    if len(path_parts) > 1:
                        feature += f"_{path_parts[1]}"  # Ajouter la seconde partie si elle existe

                    await analytics_service.track_feature_usage(
                        feature=feature, platform=platform, user_id=user_id
                    )
                ...
                if (
                    ...
                    await analytics_service.track_session_duration(
                        duration_seconds=duration, platform=platform, user_id=user_id
                    )
