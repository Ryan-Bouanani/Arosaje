...
def main():
    ...
    if run_command("python -m pytest tests/integration/ -v -m integration", "Tests d'Intégration"):
        success_count += 1
    total_tests += 1
    ...
    user_choice = input("Voulez-vous exécuter les tests Tavern? (y/N): ").lower().strip()
    if user_choice in ['y', 'yes', 'oui']:
        if run_command("python -m pytest tests/workflows/ -v --tb=short", "Tests de Workflow (Tavern)"):
            success_count += 1
        total_tests += 1
    else:
    ...
    if coverage_available:
        if run_command("python -m pytest tests/unit/ tests/integration/ --cov=. --cov-report=html --cov-report=term", "Tests avec Couverture de Code"):
