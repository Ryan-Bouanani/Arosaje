...
class PlantCareAdvice(Base):
    ...
    previous_version_id = Column(Integer, ForeignKey("plant_care_advice.id", ondelete="SET NULL"), nullable=True)
    ...
    botanist = relationship("User", foreign_keys=[botanist_id], back_populates="given_advice")
    validator = relationship("User", foreign_keys=[validator_id], back_populates="validated_advice")
    previous_version = relationship("PlantCareAdvice", remote_side=[id], back_populates="updated_versions")
    updated_versions = relationship("PlantCareAdvice", back_populates="previous_version")
    ...
    def to_dict(self):
        ...
        return {
            ...
            "validation_status": self.validation_status.value if self.validation_status else "pending",
