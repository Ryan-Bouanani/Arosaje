...
class CareReportsOwnerScreen extends StatefulWidget {
  const CareReportsOwnerScreen({super.key});

  @override
  State<CareReportsOwnerScreen> createState() => _CareReportsOwnerScreenState();
}

class _CareReportsOwnerScreenState extends State<CareReportsOwnerScreen> {
  ...
  Future<void> _loadReports() async {
    ...
    try {
      ...
      final reports = await _careReportService.getCareReportsForMyPlants();
  ...
  Map<String, List<Map<String, dynamic>>> _groupReportsByPlant() {
    final grouped = <String, List<Map<String, dynamic>>>{};
    for (final report in _reports) {
      final plantName = report['plant_name'] ?? 'Plante inconnue';
      if (!grouped.containsKey(plantName)) {
        grouped[plantName] = [];
      }
      grouped[plantName]!.add(report);
    }
    return grouped;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Rapports de mes Plantes'),
        ...
        : _error != null
          ...
          : RefreshIndicator(
              ...
              child: _reports.isEmpty
                ? const Center(
                    child: Text(
                      'Aucun rapport disponible pour vos plantes',
                      style: TextStyle(fontSize: 16, color: Colors.grey),
                    ),
                  )
                : ListView(
                    padding: const EdgeInsets.all(16),
                    children: _groupReportsByPlant().entries.map((entry) {
                      ...
                      return Card(
                        ...
                        child: ExpansionTile(
                          ...
                          subtitle: Text('${reports.length} rapport${reports.length > 1 ? 's' : ''}'),
