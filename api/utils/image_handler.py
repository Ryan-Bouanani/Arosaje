...
class ImageHandler:
    ...
    def sanitize_filename(filename: str) -> str:
        ...
        filename = re.sub(r'[<>:"/\\|?*]', '', filename)  # Supprime caractères dangereux
    ...
    async def save_image_dual(file: UploadFile, type: str) -> Tuple[str, str, str, str]:
        ...
        try:
            ...
            with Image.open(original_filepath) as img:
                ...
                if (preview_img.size[0] > PREVIEW_SIZE[0] or preview_img.size[1] > PREVIEW_SIZE[1]):
                    preview_img.thumbnail(PREVIEW_SIZE, Image.Resampling.LANCZOS)
                
                # Sauvegarder le preview avec les bons paramètres
                if file_extension == '.png':
                    preview_img.save(str(preview_filepath), optimize=True, compress_level=6)
                else:
