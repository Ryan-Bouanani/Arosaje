def validate_care_response(
    response,
    expected_status=None,
    check_start_photo=False,
    check_end_photo=False,
    ...
    logger.info(
        f"Clés présentes: {data.keys() if isinstance(data, dict) else 'Non dictionnaire'}"
    )
    ...
    if expected_status:
        assert (
            "status" in data or "care_status" in data
        ), "Le statut est manquant dans la réponse"
    ...
    if check_start_photo:
        assert (
            "start_photo_url" in data and data["start_photo_url"]
        ), "La photo de début est manquante"

    if check_end_photo:
        assert (
            "end_photo_url" in data and data["end_photo_url"]
        ), "La photo de fin est manquante"
    ...
    if check_conversation:
        ...
        ), "L'ID de conversation ne peut pas être null"
        assert isinstance(
            data["conversation_id"], int
        ), "L'ID de conversation doit être un entier"
