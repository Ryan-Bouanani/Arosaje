...
def validate_message_response(response, expected_sender_id=None, max_length=2000):
    ...
    try:
        ...
        if data["id"]:
            try:
                ...
                raise AssertionError(
                    f"L'ID du message '{data['id']}' n'est pas un nombre valide"
                )
        ...
        if data["conversation_id"]:
            try:
                ...
                raise AssertionError(
                    f"L'ID de conversation '{data['conversation_id']}' n'est pas un nombre valide"
                )
        ...
        if data["sender_id"]:
            try:
                ...
                if data["sender_id"] != "":  # Ignorer les chaînes vides pour sender_id
                    raise AssertionError(
                        f"L'ID de l'expéditeur '{data['sender_id']}' n'est pas un nombre valide"
                    )
                else:
                    data["sender_id"] = None
    ...
    assert (
        len(data["content"]) <= max_length
    ), f"Le message dépasse la longueur maximale autorisée de {max_length} caractères"

    return True


def validate_messages_list_response(
    response, expected_count=None, expected_content=None, all_read=None
):
    ...
    ), f"Le code de statut devrait être 200, mais est {response.status_code}"
    ...
    assert isinstance(
        data, list
    ), f"La réponse devrait être une liste, mais est de type {type(data)}"
    ...
    if len(data) == 0:
        ...
        if expected_count and expected_count > 0:
            raise AssertionError(
                f"Le nombre de messages devrait être au moins {expected_count}, mais est 0"
            )
        return
        ...
        try:
            ...
            if "id" in message and message["id"]:
                try:
                    ...
                    raise AssertionError(
                        f"L'ID du message '{message['id']}' n'est pas un nombre valide"
                    )
            ...
            if "conversation_id" in message and message["conversation_id"]:
                try:
                    ...
                    raise AssertionError(
                        f"L'ID de conversation '{message['conversation_id']}' n'est pas un nombre valide"
                    )
            ...
            if "sender_id" in message and message["sender_id"]:
                try:
                    ...
                    if (
                        message["sender_id"] != ""
                    ):  # Ignorer les chaînes vides pour sender_id
                        raise AssertionError(
                            f"L'ID de l'expéditeur '{message['sender_id']}' n'est pas un nombre valide"
                        )
            ...
            assert isinstance(
                message.get("id"), int
            ), "L'ID du message doit être un entier"
            assert isinstance(
                message.get("content"), str
            ), "Le contenu du message doit être une chaîne"
            assert isinstance(
                message.get("sender_id"), (int, type(None))
            ), "L'ID de l'expéditeur doit être un entier ou null"
            assert isinstance(
                message.get("conversation_id"), int
            ), "L'ID de la conversation doit être un entier"
            assert isinstance(
                message.get("created_at"), str
            ), "La date de création doit être une chaîne"
            assert isinstance(
                message.get("is_read"), bool
            ), "is_read doit être un booléen"

            # Vérifier le statut de lecture si demandé
            if all_read is not None:
                if all_read and not message.get("is_read"):
                    raise AssertionError(
                        f"Le message {message['id']} n'est pas marqué comme lu"
                    )
                elif not all_read and message.get("is_read"):
                    raise AssertionError(
                        f"Le message {message['id']} ne devrait pas être marqué comme lu"
                    )
            ...
            raise AssertionError(
                f"Erreur de conversion des données du message: {str(e)}"
            )
        except Exception as e:
            print(f"Erreur lors de la validation du message {i + 1}: {str(e)}")
            print(f"Message problématique: {message}")
    ...
    if expected_content is not None:
        ...
        if not content_found:
            ...
            raise AssertionError(
                f"Aucun message avec le contenu '{expected_content}' n'a été trouvé"
            )
...
def validate_paginated_messages_response(
    response: Dict[str, Any], expected_page_size: int
) -> None:
...
def validate_unread_messages_response(
    response: Dict[str, Any], conversation_id: int = None
) -> None:
    ...
    for item in response:
        ...
        ), "Chaque élément devrait avoir un conversation_id"
        assert "unread_count" in item, "Chaque élément devrait avoir un unread_count"
        assert isinstance(
            item["conversation_id"], int
        ), "conversation_id devrait être un entier"
        assert isinstance(
            item["unread_count"], int
        ), "unread_count devrait être un entier"
    ...
    if conversation_id is not None:
        ...
        if not conversation_found:
            raise AssertionError(
                f"La conversation {conversation_id} n'a pas été trouvée dans la réponse"
            )


def validate_participants_response(
    ...
    ), f"Le nombre de participants devrait être {expected_count}"
    ...
    for p in response:
        try:
            ...
            raise AssertionError(
                f"Erreur de conversion de l'ID du participant: {str(e)}"
            )
    ...
    for expected_id in safe_expected_ids:
        assert (
            expected_id in participant_ids
        ), f"Le participant {expected_id} est manquant"
    ...
    for participant in response:
        ...
        for field in required_fields:
            ...
            if field == "id":
                assert isinstance(
                    participant[field], int
                ), "L'ID du participant doit être un entier"
            else:
                assert isinstance(
                    participant[field], str
                ), f"Le champ {field} doit être une chaîne"


def validate_message_with_email_notification(
    response: Dict[str, Any], recipient_email: str, sender_name: str
) -> None:
...
def validate_websocket_connection_response(response):
    ...
    ), "Le code de statut devrait être 101 (Switching Protocols)"
    assert "upgrade" in response.headers.lower(), "L'en-tête 'Upgrade' est manquant"
    assert (
        "websocket" in response.headers["upgrade"].lower()
    ), "L'en-tête 'Upgrade' devrait être 'websocket'"
    assert (
        "connection" in response.headers.lower()
    ), "L'en-tête 'Connection' est manquant"
    assert (
        "upgrade" in response.headers["connection"].lower()
    ), "L'en-tête 'Connection' devrait contenir 'upgrade'"
    return True
...
def validate_websocket_message_event(event):
    ...
    ), "Le type d'événement devrait être 'new_message'"
...
def validate_websocket_typing_event(event):
    ...
    ), "Le type d'événement devrait être 'typing_status'"
...
def validate_websocket_read_event(event):
    ...
    ), "Le type d'événement devrait être 'messages_read'"
        ...
        for item in response:
            ...
            try:
                ...
                if conv_id >= 0 and count >= 0:
                    ...
                    logger.error(
                        f"Valeurs négatives trouvées: conv_id={conv_id}, count={count}"
                    )
            except (ValueError, TypeError) as e:
...
def validate_read_status_response(response):
    ...
    assert (
        data["status"] == "success"
    ), f"Le statut devrait être 'success', mais est '{data.get('status')}'"
