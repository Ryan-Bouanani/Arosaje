...
from schemas.care_report import CareReportCreate
...
def get_care_reports_by_plant_care(db: Session, plant_care_id: int) -> List[CareReport]:
    ...
    return (
        db.query(CareReport)
        .options(
            joinedload(CareReport.botanist_advices).joinedload(
                BotanistReportAdvice.botanist
            )
        )
...
def get_care_reports_by_caretaker(db: Session, caretaker_id: int) -> List[CareReport]:
    ...
    return (
        db.query(CareReport)
        .filter(CareReport.caretaker_id == caretaker_id)
        .order_by(CareReport.session_date.desc())
        .all()
    )


def get_care_reports_for_botanist(
    db: Session, botanist_id: int, skip: int = 0, limit: int = 100
) -> List[CareReport]:
...
def get_care_reports_with_my_advice(
    db: Session, botanist_id: int, skip: int = 0, limit: int = 100
) -> List[CareReport]:
...
def get_care_reports_by_owner(db: Session, owner_id: int) -> List[CareReport]:
    ...
    return (
        db.query(CareReport)
        .join(PlantCare, CareReport.plant_care_id == PlantCare.id)
        .filter(PlantCare.owner_id == owner_id)
        .order_by(CareReport.session_date.desc())
        .all()
    )
