...
class AdviceCRUD:
    ...
    def get_plant_cares_with_advice(
        ...
        query = db.query(
            ...
            Advice, PlantCare.id == Advice.plant_care_id
        ).filter(
            Advice.is_current_version == True
        )
        ...
        for result in results:
            ...
            # Conseil actuel
            current_advice = db.query(Advice).options(
                joinedload(Advice.botanist),
                joinedload(Advice.validator)
            ).filter(
                and_(
            ...
            advice_history = db.query(Advice).options(
                joinedload(Advice.botanist),
                joinedload(Advice.validator)
            ).filter(
                Advice.plant_care_id == plant_care_id
            ).order_by(desc(Advice.created_at)).all()
    ...
    def create_advice(
        ...
        try:
            ...
            existing_advice = db.query(Advice).filter(
                and_(
    ...
    def update_advice(
        ...
        current_advice = db.query(Advice).filter(
            and_(
                Advice.id == advice_id,
                Advice.is_current_version == True,
    ...
    def validate_advice(
        ...
        advice = db.query(Advice).filter(
            and_(
                Advice.id == advice_id,
                Advice.is_current_version == True,
                Advice.botanist_id != validator_id  # Ne peut pas valider ses propres conseils
            )
        ).first()
    ...
    def get_advice_stats(self, db: Session, botanist_id: Optional[int] = None) -> AdviceStats:
        ...
        advised_plant_cares = db.query(Advice.plant_care_id).filter(
            Advice.is_current_version == True
        ).subquery()
        ...
        # Gardes avec avis
        reviewed_count = db.query(Advice).filter(
            Advice.is_current_version == True
        ).count()
        
        # Compteurs par priorité
        urgent_count = db.query(Advice).filter(
            and_(
                Advice.is_current_version == True,
        ...
        follow_up_count = db.query(Advice).filter(
            and_(
                Advice.is_current_version == True,
        ...
        pending_validation = db.query(Advice).filter(
            and_(
                Advice.is_current_version == True,
        ...
        if botanist_id:
            my_advice_count = db.query(Advice).filter(
                and_(
                    Advice.botanist_id == botanist_id,
                    Advice.is_current_version == True
                )
            ...
            my_validated_count = db.query(Advice).filter(
                and_(
                    Advice.botanist_id == botanist_id,
                    Advice.is_current_version == True,
            ...
            my_validations_done_count = db.query(Advice).filter(
                Advice.validator_id == botanist_id
            ).count()
    ...
    def get_advice_by_id(self, db: Session, advice_id: int) -> Optional[Advice]:
        ...
        return db.query(Advice).options(
            joinedload(Advice.botanist),
            joinedload(Advice.validator)
        ).filter(Advice.id == advice_id).first()
    
    def get_plant_care_advice_history(self, db: Session, plant_care_id: int) -> List[Advice]:
    ...
    def get_current_plant_care_advice(self, db: Session, plant_care_id: int) -> List[Advice]:
        ...
        subquery = db.query(
            Advice.botanist_id,
            func.max(Advice.created_at).label('max_created_at')
        ).filter(
            Advice.plant_care_id == plant_care_id
        ).group_by(Advice.botanist_id).subquery()
